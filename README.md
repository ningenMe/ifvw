...
├── parsers/
│   └── openapi/    OpenAPI -> Unified API Model のparser(将来 parsers/protobuf 等を追加)
├── cli/            CLIエントリポイント(ifvw generate 等)
└── site/
    ├── index.ts    Unified API Modelから静的サイトを生成する
    └── assets/     生成される静的サイト側のUI(ブラウザで動くTypeScript + DOM API)
```
